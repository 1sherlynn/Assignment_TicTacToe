class Game
  ...
  def initialize
    @view = View.new
    @score = { rounds_played: 0, turns: 0, drawn: 0, p1: 0, computer: 0}
    @continue_game = true
    @continue_round = true
  ...
  def main_menu
    ...
    if selection == "1" 
      ...
      @view.player_info(2)
      name = "Computer"
      @player_two = Player.new(:computer, name, "O")
  ...
  def main_loop
    ...
    while @continue_game
      ...
      while @continue_round
        ...
        until valid_input
          ...
          if @active_player.name == "Computer"
              array_to_sample = @board.select{|key, value| value == "_"}.keys
              location = array_to_sample.sample
    ...
    game_winner = @player_one.name if @score[:p1] > @score[:computer]
    game_winner = @player_two.name if @score[:computer] > @score[:p1]
...
class View
  ...
  def main_menu
    ...
    puts "{Human vs computer version}"
    puts ""
    puts ""
    puts ""
 ...
 def invalid_entry
  ...
  def player_info(player)
    ...
    puts ""
    puts "The Computer will be O"
    puts ""
    puts ""
    print "Human, please enter your name:"; end
  ...
  def instructions
    ...
    puts "Instructions:"
    puts ""
    puts "In order to win, form a consecutive line of X-X-X or O-O-O befor the computer does"
    puts ""
  ...
  def final_score(winner, score)
    ...
    puts "      "
    puts "        Player1:  #{score[:p1]}   Computer:  #{score[:computer]}"
